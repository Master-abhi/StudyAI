...
export const TopicPdfStudioModal: React.FC<TopicPdfStudioModalProps> = ({
  ...
  const [generatingProvider, setGeneratingProvider] = useState<'gemini' | 'groq' | null>(null);
  ...
  const handleGenerateNotes = async (selectedProvider: 'gemini' | 'groq' = 'gemini') => {
    ...
    try {
      setGeneratingProvider(selectedProvider);
      setErrorMessage('');
      setSuccessMessage('');
      ...
      setActiveView('preview');
      setSuccessMessage(`CG GURU Study Module generated successfully with ${selectedProvider === 'groq' ? 'Groq' : 'Gemini'}! 🎓`);
    } catch (err: any) {
  ...
  const formatNotesContent = (rawText: string) => {
    ...
    return lines.map((line, idx) => {
      ...
      if (l.startsWith('###') || l.startsWith('##')) {
        ...
        return (
          <div key={idx} className="font-black text-[#17375E] text-xs mt-2.5 mb-1 flex items-center gap-1.5">
            <span className="w-1 h-3 bg-[#E88A00] rounded-full inline-block"></span>
            <span>{title}</span>
          </div>
        );
      }
      ...
      const renderedParts = parts.map((part, pIdx) => {
        if (part.startsWith('**') && part.endsWith('**')) {
          return <strong key={pIdx} className="font-black text-[#17375E]">{part.slice(2, -2)}</strong>;
        }
        return part;
      });
      ...
      if (isBullet) {
        return (
          <div key={idx} className="flex items-start gap-2 py-0.5 text-xs text-[#202938] pl-2">
            <span className="text-[#E88A00] font-bold text-xs mt-0.5">•</span>
      ...
      return (
        <p key={idx} className="text-xs text-[#202938] leading-relaxed my-0.5">
          {renderedParts}
        </p>
      );
    });
  ...
  const handleAttachToTopic = async () => {
    try {
      ...
      if (onAttachPdf) {
        ...
        const data = await res.json();
        if (!res.ok || !data.success) {
          throw new Error(data.error || 'Failed to upload and attach PDF');
        }
      }
            ...
            <div className="max-w-4xl w-full mx-auto p-5 sm:p-6 flex flex-col gap-5">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2">
                <div className="text-[10px] text-text-muted">
                  Note: AI automatically arranges your material into standard CG GURU PDF theme with Zero Omission.
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
                  ...
                  <button
                    ...
                    onClick={() => handleGenerateNotes('gemini')}
                    className="flex-1 sm:flex-initial px-5 py-2.5 bg-gradient-to-r from-saffron to-orange-500 hover:from-orange-500 hover:to-orange-600 disabled:opacity-50 text-bg-s1 text-xs font-black uppercase rounded-xl flex items-center justify-center gap-2 shadow-lg active:scale-95 cursor-pointer transition-all border border-saffron/40"
                    title="Generate comprehensive study notes using Google Gemini"
                  >
                  ...
                  <button
                    type="button"
                    disabled={generating || !rawMaterial.trim()}
                    onClick={() => handleGenerateNotes('groq')}
                    className="flex-1 sm:flex-initial px-5 py-2.5 bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-500 hover:to-amber-600 disabled:opacity-50 text-white text-xs font-black uppercase rounded-xl flex items-center justify-center gap-2 shadow-lg active:scale-95 cursor-pointer transition-all border border-amber-500/40"
                    title="Generate comprehensive study notes using Groq Llama 3.3 (High Speed)"
                  >
                    {generatingProvider === 'groq' ? (
                      <>
                        <Loader2 className="w-4 h-4 animate-spin" />
                        <span>Groq Generating...</span>
                      </>
                    ) : (
                      <>
                    ...
                    <div key={cIdx} className="flex flex-col gap-6 bg-white p-8 rounded-2xl border border-[#D9E0E8] shadow-sm avoid-break">
                      ...
                      {Array.isArray(chap.sections) && chap.sections.map((sec: any, sIdx: number) => (
                        <div key={sIdx} className="flex flex-col gap-3 pt-2">
                          ...
                          <div className="text-xs text-[#202938] leading-relaxed font-medium pl-3.5">
